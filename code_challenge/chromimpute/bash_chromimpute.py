...
os.system('python convert_bigwig_into_wig.py')
...
for the_assay in assay_all:
    the_command += 'java -mx4000M -jar ChromImpute.jar GenerateTrainData CHALLENGE/CONVERTEDDATADIR CHALLENGE/DISTANCEDIR table_train_vali.txt grch38sizes.txt CHALLENGE/TRAINDATA0 ' + the_assay + ' &\n' 
the_command += 'wait'
...
for aaa in np.arange(int(np.ceil(num_test/num_parallel))):
    ...
    for k in np.arange(start, end):
        ...
        the_cell = the_id[:3]
        the_assay = the_id[3:]
        the_command += 'java -mx4000M -jar ChromImpute.jar Train CHALLENGE/TRAINDATA0 table_train_vali.txt CHALLENGE/PREDICTORDIR0 ' + the_cell + ' ' + the_assay + ' &\n'
    the_command += 'wait'
    print(the_command)
    os.system(the_command)
...
for aaa in np.arange(int(np.ceil(num_test/num_parallel))):
    ...
    for k in np.arange(start, end):
        ...
        the_cell = the_id[:3] 
        the_assay = the_id[3:]    
        the_command += 'java -mx4000M -jar ChromImpute.jar Apply CHALLENGE/CONVERTEDDATADIR CHALLENGE/DISTANCEDIR CHALLENGE/PREDICTORDIR0 table_train_vali.txt grch38sizes.txt CHALLENGE/OUTPUTDATA0 ' + \
        the_cell + ' ' + the_assay + ' &\n'
    the_command += 'wait'
    print(the_command)
    os.system(the_command)
