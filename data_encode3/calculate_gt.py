...
def get_args():
    ...
    parser.add_argument('-i', '--input', default='K562.bigwig', type=str, help='input DNase-seq bigwig')
...
bw_input = pyBigWig.open(args.input)
...
for the_chr in chr_all:
    x=np.array(bw_input.values(the_chr,0,chr_len[the_chr]))
    x[np.isnan(x)]=0
    if np.max(x)>the_max:
        the_max = np.max(x)
