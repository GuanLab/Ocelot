...
for the_assay in tier0+tier1+tier2+tier3:
    the_command = 'time python calculate_avg_bigwig.py -reso 25 -rg grch38 -i '
    for i in range(1,80):
        the_cell='C%03d' % i
        the_file=path0 + 'n5cut_' + the_cell + '_' + the_assay + '.bigwig'
        if os.path.isfile(the_file):
            the_command += the_file + ' '
        else:
            print('empty entry: ' + the_file)
    the_command = the_command + '-o ' + path0 + 'avg_n5cut_' + the_assay + '.bigwig &'
    print(the_command)
